"""Topological Manifold: ingestion, embedding, and ontology extraction (one-shot)."""
...
def fetch_wikipedia_articles(settings: Settings) -> list[dict[str, Any]]:
    ...
    docs: list[dict[str, Any]] = []
    for topic in WIKIPEDIA_TOPICS[:8]:  # POC: first 8 of 40-topic master list
...
def save_visual_artifacts(
    ...
    *,
    ...
    """Write L0 arrays and metadata for v1_single_pass/visualisation/plot_ontology_sphere.py."""
    ...
    if chunk_embeddings is not None:
        ...
        coordinate_space = "manifold"
        viz_dim = int(chunk_embeddings.shape[1])
    else:
        chunk_emb_path.unlink(missing_ok=True)
        coordinate_space = "ambient"
        viz_dim = embedding_dim
