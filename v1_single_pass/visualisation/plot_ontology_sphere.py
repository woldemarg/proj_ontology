...
Run `python -m v1_single_pass.main` (or v2 with KEEP_VISUAL_ARTIFACTS) first.

Usage:
    python -m v1_single_pass.visualisation.plot_ontology_sphere
    python -m v1_single_pass.visualisation.plot_ontology_sphere -o v1_single_pass/data/visualisation/ontology_sphere.html
"""
...
def main() -> None:
    ...
    if missing:
        raise FileNotFoundError(
            "Missing visualisation artifacts — run v1 (KEEP_VISUAL_ARTIFACTS=true) "
            "or v2 (--plot) first:\n  "
    ...
    chunk_emb_artifact = args.artifacts / "chunk_embeddings.npy"
    if chunk_emb_artifact.exists():
        chunk_embeddings = np.load(chunk_emb_artifact)
    elif manifest.get("coordinate_space") == "manifold":
        raise FileNotFoundError(
            f"Manifold visualisation requires {chunk_emb_artifact}. "
            "Re-run python -m v1_single_pass.main with KEEP_VISUAL_ARTIFACTS=true."
        )
    else:
    ...
    if chunk_embeddings.shape[1] != concept_embeddings.shape[1]:
        raise ValueError(
            f"Chunk embedding dim ({chunk_embeddings.shape[1]}) != "
            f"concept embedding dim ({concept_embeddings.shape[1]}). "
            "Re-run python -m v1_single_pass.main with KEEP_VISUAL_ARTIFACTS=true to refresh artifacts."
        )
